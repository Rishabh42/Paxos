...
public class TreasureIslandAppAuto implements Runnable
{
    ...
    { throw new RuntimeException("IOException while initializing process log file."); }
		...
		try{ Thread.sleep(6000); } catch (InterruptedException ie) { logger.log(Level.SEVERE, "I got InterruptedException when I was chilling after all my moves.", ie); }
		ta.keepExploring = false;
		ta.tiThread.join(1500 * numPlayers); // Wait maximum 1s for the app to process any more incomming messages that was in the queue.
